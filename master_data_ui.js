...
export function renderMasterDataView(containerId, onAdd, onExcelUpload, onDeleteSelected, onDeleteAll) {
    ...
    container.innerHTML = `
        <h2 class="text-xl font-semibold mb-4 text-sky-700">회원정보 관리</h2>
        
        <div class="mb-6 p-4 border border-slate-200 rounded-lg bg-slate-50 shadow">
            <h3 class="text-lg font-medium mb-3 text-sky-600">신규 회원 추가</h3>
            <form id="addParticipantForm" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="form-group">
                        <label for="name" class="text-sm font-medium text-slate-700">이름:</label>
                        <input type="text" id="name" name="name" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                    </div>
                    <div class="form-group">
                        <label for="gender" class="text-sm font-medium text-slate-700">성별:</label>
                        <select id="gender" name="gender" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                    ...
                    <div class="form-group">
                        ...
                        <select id="type" name="type" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                    ...
                    <div class="form-group">
                        ...
                        <select id="copyType" name="copyType" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                            <option value="소복사">소복사</option>
                            <option value="대복사">대복사</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="status" class="text-sm font-medium text-slate-700">회원 상태:</label>
                        <select id="status" name="status" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                            <option value="active">활성</option>
                            <option value="inactive">비활성</option>
                            <option value="lapsed">미납</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="nextPaymentDate" class="text-sm font-medium text-slate-700">다음 결제일:</label>
                        <input type="date" id="nextPaymentDate" name="nextPaymentDate" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                    </div>
                    <div class="form-group">
                        <label for="studentPhone" class="text-sm font-medium text-slate-700">학생 연락처:</label>
                        <input type="tel" id="studentPhone" name="studentPhone" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" placeholder="하이픈 없이 숫자만">
                    </div>
                    <div class="form-group">
                        <label for="parentPhone" class="text-sm font-medium text-slate-700">부모 연락처:</label>
                        <input type="tel" id="parentPhone" name="parentPhone" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" placeholder="하이픈 없이 숫자만">
                    </div>
                </div>
                <div class="form-group">
                    <label for="memo" class="text-sm font-medium text-slate-700">메모:</label>
                    <textarea id="memo" name="memo" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" rows="2"></textarea>
                </div>
        ...
        <div class="mb-4">
            <h3 class="text-lg font-medium text-sky-600 mb-3">회원 정보 목록</h3>
        </div>
        <div class="flex flex-col sm:flex-row gap-2 mb-4 w-full">
            <button id="filterStatusButton" class="btn btn-secondary w-full sm:w-auto">
                <i data-lucide="filter" class="mr-2 h-5 w-5"></i>상태별 보기
            </button>
        ...
        <div id="statusFilterModal" class="modal">
            <div class="modal-content w-full max-w-md">
                <h3 class="text-lg font-medium mb-4 text-sky-600">회원 상태 필터</h3>
                <div class="space-y-3">
                    <div class="flex items-center">
                        <input type="radio" id="statusAll" name="statusFilter" value="all" checked class="mr-2">
                        <label for="statusAll" class="text-sm font-medium text-slate-700">전체 회원</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="statusActive" name="statusFilter" value="active" class="mr-2">
                        <label for="statusActive" class="text-sm font-medium text-slate-700">활성 회원</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="statusInactive" name="statusFilter" value="inactive" class="mr-2">
                        <label for="statusInactive" class="text-sm font-medium text-slate-700">비활성 회원</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="statusLapsed" name="statusFilter" value="lapsed" class="mr-2">
                        <label for="statusLapsed" class="text-sm font-medium text-slate-700">미납 회원</label>
                    </div>
                </div>
                <div class="flex justify-end mt-6">
                    <button id="applyStatusFilterBtn" class="btn btn-primary">적용</button>
                    <button id="closeStatusFilterBtn" class="btn btn-secondary ml-2">취소</button>
                </div>
            </div>
        </div>
        ...
        <div id="editParticipantModal" class="modal">
            <div class="modal-content">
                <h3 class="text-lg font-medium mb-4 text-sky-600">회원 정보 수정</h3>
                <form id="editParticipantForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="editName" name="name" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                        </div>
                        <div class="form-group">
                            <label for="editGender" class="text-sm font-medium text-slate-700">성별:</label>
                            <select id="editGender" name="gender" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                                <option value="남">남</option>
                                <option value="여">여</option>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="editType" name="type" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                                <option value="초등">초등</option>
                                <option value="중등">중등</option>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="editCopyType" name="copyType" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                                <option value="소복사">소복사</option>
                                <option value="대복사">대복사</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="editStatus" class="text-sm font-medium text-slate-700">회원 상태:</label>
                            <select id="editStatus" name="status" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                                <option value="active">활성</option>
                                <option value="inactive">비활성</option>
                                <option value="lapsed">미납</option>
                            </select>
                        ...
                        <div class="form-group">
                            <label for="editNextPaymentDate" class="text-sm font-medium text-slate-700">다음 결제일:</label>
                            <input type="date" id="editNextPaymentDate" name="nextPaymentDate" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3">
                        </div>
                        <div class="form-group">
                            <label for="editStudentPhone" class="text-sm font-medium text-slate-700">학생 연락처:</label>
                            <input type="tel" id="editStudentPhone" name="studentPhone" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" placeholder="하이픈 없이 숫자만">
                        </div>
                        <div class="form-group">
                            <label for="editParentPhone" class="text-sm font-medium text-slate-700">부모 연락처:</label>
                            <input type="tel" id="editParentPhone" name="parentPhone" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" placeholder="하이픈 없이 숫자만">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="editMemo" class="text-sm font-medium text-slate-700">메모:</label>
                        <textarea id="editMemo" name="memo" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3" rows="2"></textarea>
                    </div>
    ...
    document.getElementById('closeStatusFilterBtn').addEventListener('click', () => {
        document.getElementById('statusFilterModal').classList.remove('active');
    });
    
    document.getElementById('statusFilterModal').addEventListener('click', (e) => {
        if (e.target.id === 'statusFilterModal') {
            document.getElementById('statusFilterModal').classList.remove('active');
        }
    });
        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th scope="col" class="p-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">상태</th>
                    <th scope="col" class="p-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">다음 결제일</th>
            ...
            <tbody class="bg-white divide-y divide-slate-200">
                ${participants.map(p => {
                    ...
                    <tr data-id="${p.id}">
                        ...
                        <td class="p-3 whitespace-nowrap text-sm font-medium ${statusClass}">${statusText}</td>
                        <td class="p-3 whitespace-nowrap text-sm text-slate-700">${p.nextPaymentDate || '-'}</td>
...
export function populateEditForm(participant) {
    ...
    // 새로운 필드 설정
    document.getElementById('editStatus').value = participant.status || 'active';
    document.getElementById('editNextPaymentDate').value = participant.nextPaymentDate || '';
    document.getElementById('editMemo').value = participant.memo || '';
